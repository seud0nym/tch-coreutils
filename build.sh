...
for __BINARY in cmake curl git make tar strip upx; do
...
if [ "$1" == "clean" ]; then
  ...
  rm -rf .work bin/usign toolchains
elif [ "$1" == "pkgonly" ]; then
  shift
  __PKG_ONLY=Y
fi
if [ "$1" == "buildtoolchains" ]; then
  shift
  __BUILD_TOOLCHAINS=Y
fi
...
for I in $(seq 0 $((${#__MUSL_PRFX[@]} - 1))); do
  ...
  elif [ $__BUILD_TOOLCHAINS = N ]; then
    echo -e "${GREEN}$(date +%X) ==> INFO:  Downloading $__TARGET toolchain...${GREY}[$(pwd)]${NC}"
    curl -L https://musl.cc/${__TARGET}-cross.tgz -o /tmp/${__TARGET}-cross.tgz
    echo -e "${GREEN}$(date +%X) ==> INFO:  Extracting $__TARGET toolchain...${GREY}[$(pwd)]${NC}"
    tar -xzf /tmp/${__TARGET}-cross.tgz -C toolchains
    rm -f /tmp/${__TARGET}-cross.tgz
  else
...
if [ $__PKG_ONLY == Y ]; then
  ...
  for I in $(seq 0 $((${#__MUSL_PRFX[@]} - 1))); do
    ...
    ${__MUSL_PRFX[$I]}-strip -s -R .comment -R .gnu.version --strip-unneeded $__EXE_FILES
...
for I in $(seq 0 $((${#__MUSL_ARCH[@]} - 1))); do
  ...
  for __F in $(ls releases/${__MUSL_ARCH[$I]}/* | grep -v '\['); do
    if file -b "$__F" | grep -q '^ELF'; then
